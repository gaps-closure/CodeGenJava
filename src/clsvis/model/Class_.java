...
public class Class_ extends ParameterizableElement {
    ...
    private Set<Class_> subclasses = new HashSet<>();
    private Class_ superclass;
    private int depth = 0;
    private ArrayList<ParameterizableElement> primitiveFields = new ArrayList<ParameterizableElement>();
    private ArrayList<ParameterizableElement> objFields = new ArrayList<ParameterizableElement>();
    ...
    // tchen
    public void addSubclass(Class_ sub) {
        subclasses.add(sub);
    }

    public void showSubclass() {
        System.out.println(fullTypeName);
        for (Class_ clazz : subclasses) {
            System.out.println("  " + clazz.fullTypeName);
        }
    }

    public Set<Class_> getSubclasses() {
        return subclasses;
    }

    public List<ParameterizableElement> getMember(ElementKind memberKind) {
        return membersMap.get( memberKind );
    }
    ...
    public ArrayList<ParameterizableElement> getPrimitiveFields() {
        return primitiveFields;
    }

    public ArrayList<ParameterizableElement> getObjFields() {
        return objFields;
    }
    
    public void sortFields() {
        List<ParameterizableElement> fields = getMember(ElementKind.Fields);
        if (fields != null) {
            for (ParameterizableElement field : fields) {
                if (field.getType().isPrimitive())
                    primitiveFields.add(field);
                else
                    objFields.add(field);
            }
            Collections.sort(primitiveFields, new ElementComparator());   
            Collections.sort(objFields, new ElementComparator());  
        }
    }

    class ElementComparator implements Comparator<ParameterizableElement> {
        @Override
        public int compare(ParameterizableElement o1, ParameterizableElement o2) {
